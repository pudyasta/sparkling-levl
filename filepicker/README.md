...
This directory is copied when running `pnpm dlx sparkling-method-cli init <name>`. It
...
After editing the `.d.ts` declarations, run `pnpm dlx sparkling-method-cli codegen` (or
...
following the same patterns used in `sparkling-navigation` and `sparkling-storage`.
