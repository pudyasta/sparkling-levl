...
final class BackInterceptorMethod: PipeMethod {
    ...
    /// Fire the `nativeBackPressed` event to all active Lynx containers.
    /// The hosting view controller calls this instead of actually navigating back.
    static func dispatchNativeBackEvent() {
        guard isInterceptEnabled else { return }
        let params: [String: Any] = ["source": "ios_back"]
        // LynxPipeEnginePool.enumerateKeysAndObjects { _, engine, _ in
        //     (engine as? PipeEngine)?.fireEvent(name: "nativeBackPressed", params: params)
        // }
    }
}
