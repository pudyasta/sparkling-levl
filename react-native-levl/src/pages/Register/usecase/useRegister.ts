...
export const useRegister = (options?: UseRegisterOptions) => {
  ...
  const mutation = useMutation<AuthResponse, any, RegisterRequest>({
    mutationFn: async (rawValues: RegisterRequest) => {
      ...
      if (!result.success) throw { type: 'VALIDATION_ERROR', errors: result.errors };
      const response = await registerApi(rawValues);
      return response.data;
    },
    onSuccess: (data: AuthResponse) => {
      if (data.message === 'messages.validation.failed') {
        const err = {
          email: (data.errors as any)?.email?.[0] ?? null,
          password: (data.errors as any)?.password?.[0] ?? null,
          password_confirmation: (data.errors as any)?.password_confirmation?.[0] ?? null,
          username: (data.errors as any)?.username?.[0] ?? null,
          name: (data.errors as any)?.name?.[0] ?? null,
        };
