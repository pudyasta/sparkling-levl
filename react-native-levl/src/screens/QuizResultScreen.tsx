...
export default function QuizResultScreen() {
  ...
  const final_score = Number(params.final_score);
  const is_passed = params.is_passed === 'true';
  const time_spent = Number(params.time_spent);
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.scoreBadge, { backgroundColor: is_passed ? '#E8F5E9' : '#FFEBEE' }]}>
          <AppText
            size={TextType.h1}
            fontWeight="bold"
            color={is_passed ? Colors.Success : Colors.Error}
          >
            {final_score}
          </AppText>
          <AppText size={TextType.b2} color={is_passed ? Colors.Success : Colors.Error}>
            Final Score
          </AppText>
        </View>
...
const styles = StyleSheet.create({
  ...
  scoreBadge: { borderRadius: 20, padding: 24, alignItems: 'center', width: '100%' },
  statsRow: {
    flexDirection: 'row',
    gap: 24,
    backgroundColor: '#fff',
