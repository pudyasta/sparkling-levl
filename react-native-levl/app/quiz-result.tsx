...
function ScoreRing({ percentage, isPassed }: { percentage: number; isPassed: boolean }) {
  ...
  return (
    <View style={[styles.scoreRing, { borderColor: color }]}>
      <Text size={TextType.h1} fontWeight="bold" fontFamily={FontFamily.jakarta} color={color} style={{ fontSize: 36 }}>
        {Math.round(percentage)}
      </Text>
      <Text size={TextType.b3} color={color}>/ 100</Text>
    </View>
  );
}
...
export default function QuizResultScreen() {
  ...
  useEffect(() => {
    ...
    repo.getQuizOverviewApi(parseInt(submissionId))
  ...
  if (!overview) {
    return (
      <View style={styles.center}>
        <Text size={TextType.b2} color={Colors.Error}>Gagal memuat hasil quiz.</Text>
        <Button onPress={() => router.back()} style={{ marginTop: 16 }}>Kembali</Button>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: Colors.Canvas }} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        <Text size={TextType.h1} fontWeight="bold" fontFamily={FontFamily.jakarta} style={{ textAlign: 'center', marginTop: 8 }}>
        ...
        <View style={styles.statsRow}>
          {overview.correct_count !== undefined && (
            <View style={styles.statItem}>
              <Text size={TextType.h2} fontWeight="bold" color={Colors.Success}>{overview.correct_count}</Text>
              <Text size={TextType.b3} color={Colors.TextTertiary}>Benar</Text>
            </View>
          )}
          {overview.wrong_count !== undefined && (
            <View style={styles.statItem}>
              <Text size={TextType.h2} fontWeight="bold" color={Colors.Error}>{overview.wrong_count}</Text>
              <Text size={TextType.b3} color={Colors.TextTertiary}>Salah</Text>
            </View>
          )}
          {overview.total_questions !== undefined && (
            <View style={styles.statItem}>
              <Text size={TextType.h2} fontWeight="bold" color={Colors.Primary}>{overview.total_questions}</Text>
              <Text size={TextType.b3} color={Colors.TextTertiary}>Total</Text>
            </View>
          )}
        </View>
      </View>
        ...
        <View style={{ gap: 12 }}>
          ...
          {answers.map((item: any, idx: number) => {
            ...
            return (
              <View
                key={item.id ?? idx}
                style={[styles.answerCard, { borderLeftColor: isCorrect ? Colors.Success : Colors.Error }]}
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                  <Text size={TextType.b3} color={Colors.TextTertiary}>Soal {idx + 1}</Text>
