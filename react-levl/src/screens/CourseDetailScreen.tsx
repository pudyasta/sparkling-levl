...
type RouteT = RouteProp<RootStackParamList, 'CourseDetail'>;

export default function CourseDetailScreen() {
  const navigation = useNavigation<Nav>();
  const { params } = useRoute<RouteT>();
  const { course, isLoading, refetch } = useGetCourseDetail(params.slug);

  if (isLoading) return <CourseDetailSkeleton onBack={() => navigation.goBack()} />;
                ...
                onPress={() => {
                  ...
                  if (firstUnit && firstLesson) {
                    navigation.navigate('Lessons', {
                      ...
                      all_lessons: firstUnit.elements ?? [],
                    });
                  }
                }}
              />
...
function CourseDetailSkeleton({ onBack }: { onBack: () => void }) {
  return (
