...
type RouteT = RouteProp<RootStackParamList, 'Lessons'>;
...
export default function LessonsScreen() {
  ...
  const allLessons = useRef<any[]>(params.all_lessons ?? []);
  ...
  const handleMarkDone = () => {
    ...
    markDone(currentParams.lesson_slug, {
      onSuccess: (data: any) => {
        ...
        const done = allLessons.current.filter((l: any) => l.is_completed).length;
        setCompletedLessons(done);
        setProgressPercentage(allLessons.current.length > 0 ? (done / allLessons.current.length) * 100 : 0);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <ScrollView style={{ flex: 1 }}>
            {allLessons.current.map((item: any, i: number) => {
              ...
              return (
                ...
                >
                  {item.is_completed ? (
                    <View style={styles.dotDone}><Text style={{ color: '#fff', fontSize: 10 }}>✓</Text></View>
                  ) : (
                    <View style={[styles.dot, isActive && { borderColor: Colors.Primary }]} />
                  )}
                  <Text style={[styles.lessonTitle, isActive && { color: Colors.Primary, fontWeight: '700' }]}>
                    {item.title}
                  </Text>
                </TouchableOpacity>
              );
      ...
      >
        <View style={{ padding: 8, gap: 16 }}>
          ...
          <AppButton label="Keluar" color="primary" onPress={() => { setIsBackModal(false); navigation.goBack(); }} />
          <AppButton label="Tetap di sini" variant="outlined" color="primary" onPress={() => setIsBackModal(false)} />
        </View>
      </AppModal>
    </SafeAreaView>
  );
